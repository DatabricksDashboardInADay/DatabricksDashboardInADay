...
SELECT
  *
FROM
  STREAM read_files('/Volumes/${catalog}/bronze/raw/dim_date/', format => 'csv');
...
SELECT
  *
FROM
  STREAM read_files('/Volumes/${catalog}/bronze/raw/dim_store/', format => 'csv');
...
SELECT
  *
FROM
  STREAM read_files('/Volumes/${catalog}/bronze/raw/dim_customer/', format => 'csv');
...
SELECT
  *
FROM
  STREAM read_files('/Volumes/${catalog}/bronze/raw/dim_product/', format => 'csv');

-- Coffee sales fact (Parquet -> silver streaming)
CREATE OR REFRESH STREAMING TABLE silver.fact_coffee_sales
  ( CONSTRAINT valid_quantity EXPECT(quantity_sold > 0) ON VIOLATION DROP ROW) AS
SELECT
  *
FROM
  STREAM read_files('/Volumes/${catalog}/bronze/raw/fact_coffee_sales/', format => 'parquet');